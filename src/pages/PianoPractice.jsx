...
// ... (keep your keyMap, noteToKey, and allKeys constants the same)
const keyMap = {
    // Bottom row
...
export default function PianoPractice() {
    // Get the mode from the URL (e.g., 'ranked' or 'unranked')
    const { mode } = useParams();

    // Use useRef for the sampler to avoid re-creating it on re-renders
    ...
    const playNote = useCallback((note) => {
        if (sampler.current && isLoaded && !activeNotes.has(note)) {
            sampler.current.triggerAttack(note, Tone.now());
            setActiveNotes(prev => new Set(prev.add(note)));
        }
    }, [isLoaded, activeNotes]);
    ...
    const stopNote = useCallback((note) => {
        if (sampler.current && isLoaded && activeNotes.has(note)) {
            ...
            // For a hard stop, you might use triggerAttackRelease, but that is often less natural.
            // This implementation lets the note ring out.
            setActiveNotes(prev => {
    ...
    // Keyboard event handlers (no changes needed here)
    useEffect(() => {
        ...
        <section>
            <div className="piano-container" onClick={startAudio}>
                <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 'var(--space-md)' }}>
                    <h2>Play Piano</h2>
                    <span className="badge" style={{ textTransform: 'capitalize' }}>Unranked Mode</span>
                </div>
                <p className="muted">
                    {isLoaded ? 'Use your keyboard to play. Click the piano to enable audio.' : 'Loading piano samples...'}
                </p>
                <div className={`piano ${!isLoaded ? 'loading' : ''}`}>
                    {allKeys.map(({ note, type }) => (
                        <div
                            key={note}
                            data-note={note}
                            className={`piano-key ${type} ${activeNotes.has(note) ? 'active' : ''}`}
                            onMouseDown={() => isLoaded && playNote(note)}
                            onMouseUp={() => isLoaded && stopNote(note)}
                            onMouseLeave={() => isLoaded && stopNote(note)}
                        >
